...
import { getPurchaseInvoicesByParty, savePaymentDone } from "../utils/storage";
import { PaymentDone, PaymentAllocation } from "../utils/types";
import { toast } from "sonner";
import { Checkbox } from "./ui/checkbox";

export default function RecordPaymentDone() {
  const { partyName } = useParams();
  const navigate = useNavigate();
  const decodedPartyName = decodeURIComponent(partyName || "");

  const invoices = getPurchaseInvoicesByParty(decodedPartyName).filter(inv => inv.outstanding > 0);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    savePaymentDone(payment);
    toast.success(`Payment of ₹${amount.toLocaleString('en-IN')} recorded successfully!`);
    navigate(`/party/purchase/${encodeURIComponent(decodedPartyName)}`);
  };
        ...
        <header className="bg-white border-b">
          <div className="container mx-auto px-4 py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Record Payment Done</h1>
        ...
        <main className="container mx-auto px-4 py-8">
          <Card>
            <CardContent className="text-center py-12">
              ...
              <Link to={`/party/purchase/${encodeURIComponent(decodedPartyName)}`}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {invoices.map((invoice) => (
                <div
                  key={invoice.id}
                  className={`p-4 border rounded-lg ${selectedInvoices.has(invoice.id) ? 'border-red-500 bg-red-50' : 'border-gray-200'}`}
                >
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <div className="flex gap-3">
                <Link to={`/party/purchase/${encodeURIComponent(decodedPartyName)}`} className="flex-1">
